...
unsigned int get_highscore()
{
  ...
  {
    EEPROM.get(EEPROM_SAVE_START + i, temp);
    if (magic[i] != temp)
    {
      for (int j = 0 ; j < 3 ; j++)
        EEPROM.put(EEPROM_SAVE_START + j, magic[j]);
      highscore = 0;
      return;
    }
  }
  EEPROM.get(EEPROM_SAVE_START + 3, highscore);
}

void set_highscore()
{
  EEPROM.put(EEPROM_SAVE_START + 3, highscore);
}
